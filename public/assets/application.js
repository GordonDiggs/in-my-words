...
(function() {
  ...
  $(function() {
    var WORDS, replaceIfBannz0rred;
    _.mixin(_.str.exports());
    WORDS = new app.OptionsCollection().instance().get('dictionary');
    replaceIfBannz0rred = function(word) {
      if (WORDS[word] != null) {
        return WORDS[word];
      } else {
        return word;
    ...
    return _(WORDS).each(function(replacement, original) {
      return $("*").each(function(el, i) {
        var woot;
        if ($(this).children().length === 0) {
          woot = _($(this).text()).chain().words().reduce(function(memo, word) {
            return "" + memo + " " + (replaceIfBannz0rred(word));
          }, "").value();
          return $(this).text(woot);
        }
      });
    });
  });
...
(function() {
  ...
  app.OptionsCollection = (function(_super) {

    __extends(OptionsCollection, _super);

    function OptionsCollection() {
      OptionsCollection.__super__.constructor.apply(this, arguments);
    }

    OptionsCollection.prototype.model = app.Options;

    OptionsCollection.prototype.localStorage = new Backbone.LocalStorage("Options");

    OptionsCollection.prototype.initialize = function() {
      return this.fetch();
    };

    OptionsCollection.prototype.instance = function() {
      return this.at(0) || this.add({}).at(0);
    };
  ...
  save = function(options) {
    $inputs().each(function(i, el) {
      var val;
      val = $(el).val();
      if ($(el).hasClass('json')) val = JSON.parse(val);
      return options.set($(el).attr('name'), val);
    });
    return options.save();
  };

  restore = function(options) {
    return $inputs().each(function(i, el) {
      var val;
      val = options.get($(el).attr('name'));
      if ($(el).hasClass('json')) val = JSON.stringify(val);
      return $(el).val(val);
    });
  };
